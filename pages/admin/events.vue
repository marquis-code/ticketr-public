  ...
  <div class="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div class="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 w-full md:w-auto">
          ...
          <nav class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide text-xs font-semibold">
            ...
            <NuxtLink to="/admin/events" class="bg-primary text-white px-3 py-1.5 rounded-lg whitespace-nowrap">Events</NuxtLink>
            <NuxtLink to="/admin/orders" class="text-gray-600 hover:text-gray-900 px-3 py-1.5 rounded-lg transition whitespace-nowrap">Orders</NuxtLink>
          </nav>
        </div>
    ...
    <main class="max-w-7xl mx-auto px-4 md:px-6 py-8 flex-grow w-full space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div v-if="loading" class="text-center py-12 text-gray-500 text-sm">Loading events...</div>
        <div v-else-if="events.length === 0" class="text-center py-12 text-gray-500 text-sm">No events yet.</div>
          ...
          <table class="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
              <tr v-for="ev in events" :key="ev._id" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 font-semibold text-gray-900">₦{{ (ev.totalRevenue || 0).toLocaleString() }}</td>
                <td class="px-6 py-4">
                  <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-600 border border-emerald-200">{{ ev.status }}</span>
                </td>
              </tr>
            </tbody>
          </table>
