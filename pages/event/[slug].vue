  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
    ...
    <header class="glass-card border-b border-slate-800/80 px-6 py-4">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <NuxtLink :to="`/?tenant=${tenantSlug}`" class="text-sm font-semibold text-slate-400 hover:text-white transition flex items-center gap-2">
          ← Back to Events
        </NuxtLink>
    ...
    <main class="max-w-7xl mx-auto px-6 py-8 flex-grow w-full">
      ...
      <div v-if="loading" class="text-center py-24">
        ...
        <p class="text-slate-400 text-sm mt-4">Loading event details...</p>
      </div>

      <!-- Error / Not Found -->
      <div v-else-if="!eventData || !eventData.event" class="glass-card rounded-2xl p-12 text-center max-w-md mx-auto my-12">
        <p class="text-xl font-bold text-white mb-2">Event Not Found</p>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="glass-card rounded-2xl overflow-hidden">
            ...
            <div class="p-6 md:p-8 space-y-6">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 text-lg">
                    📅
                  </div>
                  <div>
                    <span class="text-xs text-slate-500 block uppercase font-medium">Date & Time</span>
                    <span class="text-sm font-semibold text-white">{{ formatDate(eventData.event.startDate) }}</span>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <span class="text-xs text-slate-500 block uppercase font-medium">Location</span>
                    <span class="text-sm font-semibold text-white truncate max-w-[200px] block">{{ eventData.event.location }}</span>
                  </div>
                </div>
              </div>

              <div>
                <h3 class="text-lg font-bold text-white mb-3">About This Event</h3>
        ...
        <div class="space-y-6">
          <div class="glass-card rounded-2xl p-6 md:p-8 border-indigo-500/30 sticky top-24">
            <h2 class="text-xl font-bold text-white mb-4">Select Tickets</h2>
            ...
            <div class="space-y-4 mb-6">
              <div
                ...
                :class="selectedQuantities[tier._id] > 0 ? 'bg-indigo-950/30 border-indigo-500/60' : 'bg-slate-900/40 border-slate-800'"
              >
                <div class="flex items-center justify-between mb-2">
                  <div>
                    <h4 class="font-bold text-white text-base">{{ tier.name }}</h4>
                    <span class="text-xs text-slate-400 block">{{ tier.capacity - tier.soldCount }} tickets remaining</span>
                  </div>
                ...
                <div class="flex items-center justify-between pt-3 border-t border-slate-800/60 mt-2">
                  <span class="text-xs text-slate-500">Qty (Max {{ tier.maxPerPurchase }})</span>
                  <div class="flex items-center gap-3">
                    <button
                      @click="updateQuantity(tier._id, -1, tier.maxPerPurchase)"
                      :disabled="!selectedQuantities[tier._id]"
                      class="w-8 h-8 rounded-lg bg-slate-800 text-slate-300 font-bold hover:bg-slate-700 disabled:opacity-30 disabled:pointer-events-none transition"
                    >
                      -
                    </button>
                    <span class="font-bold text-white text-sm w-4 text-center">{{ selectedQuantities[tier._id] || 0 }}</span>
                    <button
                      @click="updateQuantity(tier._id, 1, tier.maxPerPurchase)"
                      :disabled="(selectedQuantities[tier._id] || 0) >= Math.min(tier.maxPerPurchase, tier.capacity - tier.soldCount)"
                      class="w-8 h-8 rounded-lg bg-indigo-600 text-white font-bold hover:bg-indigo-500 disabled:opacity-30 disabled:pointer-events-none transition"
                    >
                      +
                    </button>
                  </div>
            ...
            <div class="space-y-4 pt-4 border-t border-slate-800 mb-6">
              <h4 class="font-semibold text-white text-sm">Attendee Contact Info</h4>
              ...
              <div>
                <label class="block text-xs font-medium text-slate-400 mb-1">Department / Association Code</label>
                <input
                  v-model="departmentCode"
                  type="text"
                  placeholder="e.g. EDM, TVESA, ULSESA"
                  class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white uppercase focus:outline-none focus:border-indigo-500 transition"
                />
                <span class="text-[11px] text-slate-500 mt-1 block">Used to generate structured ticket code (e.g. V/T01/EDM)</span>
              </div>
            </div>
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="text-2xl font-extrabold text-white">₦{{ totalAmount.toLocaleString() }}</span>
              </div>
