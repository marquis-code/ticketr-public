  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6">
    <div class="w-full max-w-lg">
      ...
      <div v-if="loading" class="glass-card rounded-2xl p-12 text-center">
        ...
        <h3 class="text-lg font-bold text-white mt-4">Verifying Payment & Generating Tickets...</h3>
      ...
      <div v-else-if="error" class="glass-card rounded-2xl p-8 text-center border-rose-500/40">
        ...
        <h3 class="text-xl font-bold text-white mb-2">Payment Verification Error</h3>
      ...
      <div v-else-if="orderData" class="space-y-6">
        <div class="text-center">
          ...
          <h2 class="text-2xl font-extrabold text-white">Payment Confirmed!</h2>
          <p class="text-sm text-slate-400 mt-1">Order #{{ orderData.orderNumber }}</p>
        </div>
        ...
        <div v-for="ticket in orderData.tickets" :key="ticket._id" class="glass-card rounded-2xl overflow-hidden border-indigo-500/40 print:bg-white print:text-black">
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span class="text-xs text-slate-500 uppercase font-medium block">Attendee</span>
                <span class="font-semibold text-white">{{ ticket.attendeeName }}</span>
              </div>
              <div>
                <span class="text-xs text-slate-500 uppercase font-medium block">Email</span>
                <span class="font-semibold text-slate-300 text-xs truncate block">{{ ticket.attendeeEmail }}</span>
              </div>
            </div>

            <!-- QR Code Render -->
            <div class="bg-slate-900 rounded-xl p-6 text-center border border-slate-800 my-4">
